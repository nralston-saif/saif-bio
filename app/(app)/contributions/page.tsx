...
export default async function ContributionsPage() {
  ...
  const [contributionsRes, contactsRes, lettersRes, stockDetailsRes] = await Promise.all([
    supabase
    ...
    supabase.from('bio_stock_contribution_details').select('*'),
  ])
  ...
  const stockDetails = (stockDetailsRes.data ?? []) as unknown as StockContributionDetail[]
  ...
  const stockDetailByContribution = new Map<string, StockContributionDetail>(
  ...
  const ytd = contributions.filter(
    (c) =>
      c.received_date.startsWith(`${currentYear}-`) &&
      c.method !== 'in_kind' &&
      c.method !== 'stock'
  )
  const ytdCashCents = ytd.reduce((sum, c) => sum + (c.amount_cents ?? 0), 0)
  const ytdStock = contributions.filter(
      ...
      <div className="card p-5 mb-6 grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div>
          ...
          <p className="text-sm text-gray-500">
            {currentYear} · {ytd.length} gift{ytd.length === 1 ? '' : 's'}
          </p>
        </div>
        <div>
          <p className="text-xs text-gray-500 uppercase tracking-wide">Stock FMV</p>
          <p className="text-2xl font-semibold text-gray-900 tabular-nums">
            {formatCents(ytdStockCents)}
          </p>
          <p className="text-sm text-gray-500">
            {currentYear} · {ytdStock.length} gift{ytdStock.length === 1 ? '' : 's'}
          </p>
        </div>
      </div>
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {contributions.map((c) => {
                ...
                const stockDetail = stockDetailByContribution.get(c.id)
                return (
                  <tr key={c.id} className="border-b border-gray-50 hover:bg-gray-50">
                    ...
                    <td className="px-4 py-3 text-gray-600">
                      {c.method === 'stock' && stockDetail ? (
                        <span>
                          Stock · {formatShares(stockDetail.shares)}{' '}
                          {stockDetail.ticker_symbol ?? 'shares'}
                        </span>
                      ) : (
